"""Live composition root for vibetrading_V2."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run vibetrading_V2 live runtime")
    ...
    ports = RunnerPorts(
        ...
        execution=LiveExecutionAdapter(clock),
        clock=clock,
        logger=_PrintLogger(),
    )

    result = run_live(args.strategy, ports=ports, strategies_dir=args.strategies_dir)
    print(f"[live] strategy={result.strategy_name} orders={result.orders_submitted}")
    return 0
