...
Project: VibeTrading modular strategy plugin refactor (Python)

Non-negotiables:
- Single editable surface: user edits ONLY `strategies/my_strategy_a.py`
- Strategy may declare: meta (universe/timeframe/features) + cost/risk/sizing overrides
- Defaults live in `vibetrading/policies/*`; strategy overrides are partial merges
- `strategies/*` imports allowlist only: `vibetrading.core.*`, `vibetrading.strategy.(base|bundle)` (+ optionally numpy/pandas)
- `strategies/*` denylist: `vibetrading.runner.*`, `vibetrading.execution.*`, `vibetrading.data.*`, `cli/*`, any direct I/O / broker SDK
- Runtime data is read-only parquet under `data/curated`

Architect deliverables (must be written):
- `.omx/plans/vibetrading_refactor_prd.md`
- `.omx/plans/vibetrading_import_hotspots.md`

Executor rule:
- MUST read both files first; if missing, stop and request architect pass.

Canonical spec: docs/vibetrading_refactor_spec.md
Architect/Executor must read this spec first.
</project_constitution>
...
Architect→Executor Handoff (mandatory):
- Architect must write `.omx/plans/vibetrading_refactor_prd.md` and `..._import_hotspots.md`
- Executor must read and summarize them BEFORE edits; if missing/outdated, stop and re-run architect.  
</execution_protocols>
...
You are a team worker in team "read-omx-plans-vibetrading-ref". Your identity and assigned tasks are in your inbox file.
...
4. Read your task from .omx/state/team/read-omx-plans-vibetrading-ref/tasks/task-<id>.json (example: task-1.json)
5. Task id format:
...
9. Update your status: write {"state": "idle"} to .omx/state/team/read-omx-plans-vibetrading-ref/workers/{your-name}/status.json
10. Wait for new instructions (the lead will send them via your terminal)
11. Check your mailbox for messages at .omx/state/team/read-omx-plans-vibetrading-ref/mailbox/{your-name}.json
