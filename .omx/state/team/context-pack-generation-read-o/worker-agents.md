...
Project: VibeTrading V2 (Python) — clean modular auto-trading system with swappable data/broker adapters.

Repository reality:
- `vibetrading_v1/` exists and may run, but it is NOT a correctness oracle.
- `vibetrading_v2/` is the target system to complete (primary focus).

Scope (current):
- Market data: **1-minute OHLCV bars only** (NO orderbook).
- Execution: **paper + live** must both be supported through one Broker port.
- Backtesting: via **Parquet** (primary deterministic) and optionally Kiwoom historical download.
- Strategy families (in scope now):
  1) Portfolio Optimization & Rebalancing
  2) Single Strategy Trading
- Arbitrage: **out of scope** for now; define stub interfaces only (no live WS dependency).

Non-negotiable architecture:
- **Pure core**: `vibetrading_v2/core/**` MUST NOT import network/DB/broker SDKs or do direct I/O.
  - Disallowed in core: `requests`, websocket clients, DB drivers, broker SDKs, filesystem writes (except via injected ports).
- **Ports/Adapters**:
  - `vibetrading_v2/ports/**` defines interfaces ONLY:
    - `BarDataSource` (historical + live bars)
    - `Broker` (paper + live)
    - `Clock` (live vs backtest)
    - `StateStore` (optional)
  - `vibetrading_v2/adapters/**` contains all external integrations:
    - `datasource_parquet.py`
    - `datasource_kiwoom_historical.py` (optional)
    - `datasource_kis_live.py` (optional; bar aggregation rules explicit)
    - `broker_paper.py`
    - `broker_kiwoom.py` / `broker_kis.py` (optional)
- Apps/entrypoints live outside core:
  - `vibetrading_v2/apps/backtest.py`
  - `vibetrading_v2/apps/trade_paper.py`
  - `vibetrading_v2/apps/trade_live.py`

Safety (live trading):
- Default mode is **paper**. Live trading must be opt-in and gated.
- Any code path that can place live orders must require BOTH:
  - env `LIVE_API=1`
  - env `CONFIRM_LIVE=YES`
- If either is missing: force paper, or hard-fail with a clear message.
- Never print secrets. Never commit `.env`. `.env.example` contains keys only (no values).

Data semantics (bars):
- Bar timestamps, timezone, and bar close rules MUST be explicit and consistent across sources:
  - `Bar` schema must define `ts` meaning (close time recommended).
  - Handle missing/duplicate/out-of-order bars deterministically.

Order semantics:
- Core must implement a broker-agnostic order lifecycle:
  - idempotency key required for all orders
  - state machine for (Created → Submitted → Accepted/Rejected → PartiallyFilled → Filled/Cancelled/Expired)
- Paper broker must mirror the same lifecycle shape (even if fills are simulated).

Verification and Definition of Done (DoD):
- Primary DoD is **deterministic backtest**:
  - `ParquetDataSource + PaperBroker` run produces artifacts (orders/positions/pnl/risk)
  - Re-running with same input+config must produce identical artifact hashes (or explicitly defined tolerances)
- Live API is secondary and optional:
  - smoke test only (connect, fetch bars, place 1 tiny order in sandbox/paper where supported, cancel, disconnect)
  - must be protected by `LIVE_API=1` and `CONFIRM_LIVE=YES`

Planning-first protocol (to avoid missing repo context):
- Before writing a “final-level” plan or making broad refactors:
  1) Generate a **Context Pack** under `docs/context/` (read-only analysis; no code edits).
  2) Then create a consensus plan in `.omx/plans/` grounded ONLY on that context pack.

Architect deliverables (must be written before major code changes):
- `docs/context/` Context Pack files (see below).
- `.omx/plans/v2_master_plan.md`
- `.omx/plans/v2_lane_ownership.md` (directory/file ownership boundaries for team parallelism)
- `.omx/plans/v2_acceptance_commands.md` (exact commands + expected outputs evidence)

Context Pack required files (minimum):
- `docs/context/00_repo_overview.md`
- `docs/context/10_v1_system_map.md`
- `docs/context/20_v2_system_map.md`
- `docs/context/30_gap_matrix.md`
- `docs/context/40_interfaces_and_schemas.md`
- `docs/context/50_bar_semantics.md`
- `docs/context/60_order_and_safety_semantics.md`
- `docs/context/70_test_and_verification_matrix.md`
- `docs/context/80_risk_register.md`

Executor rule:
- MUST read and summarize the above deliverables before editing code.
- If missing or stale: stop and request an architect/planner pass to regenerate.
</project_constitution>
...
Architect→Executor Handoff (mandatory for major refactors/implementation):
- Architect must write:
  - `docs/context/*` Context Pack
  - `.omx/plans/v2_master_plan.md`
  - `.omx/plans/v2_lane_ownership.md`
  - `.omx/plans/v2_acceptance_commands.md`
- Executor must read and summarize them BEFORE edits; if missing/outdated, stop and re-run an architect/planner pass.
</execution_protocols>
...
You are a team worker in team "context-pack-generation-read-o". Your identity and assigned tasks are in your inbox file.
...
4. Read your task from .omx/state/team/context-pack-generation-read-o/tasks/task-<id>.json (example: task-1.json)
5. Task id format:
...
9. Update your status: write {"state": "idle"} to .omx/state/team/context-pack-generation-read-o/workers/{your-name}/status.json
10. Wait for new instructions (the lead will send them via your terminal)
11. Check your mailbox for messages at .omx/state/team/context-pack-generation-read-o/mailbox/{your-name}.json
