...
- `scripts/run_backtest.py` now supports `--team {auto,portfolio,trading,arbitrage}` for both `backtest` and `walkforward`.
- Default `--team auto` resolves the team from strategy `TEAM_TYPE` and picks the exact engine automatically.
- If user manually sets `--team`, it is validated against strategy `TEAM_TYPE` to prevent wrong-engine backtests.
...
- Existing workflows without explicit `team` now use `--team auto` and detect team from strategy metadata.
...
### Strategy file contract for automatic engine selection
```python
from shared.models import TeamType

TEAM_TYPE = TeamType.PORTFOLIO  # or TeamType.TRADING / TeamType.ARBITRAGE
```
